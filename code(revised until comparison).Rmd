...
<!-- $\bar{u}_{\text{ap_hi}}$, $\bar{u}_{\text{ap_lo}}$ -->
...
vanilla.CI<- data.frame(variable = character(0), est = numeric(0), se = numeric(0), lower = numeric(0), upper = numeric(0))
input_values <- list(
  ap_hi = list(est = ap_hi.est, se = ap_hi.se),
  ap_lo = list(est = ap_lo.est, se = ap_lo.se),
  cardio = list(est = cardio.est, se = cardio.se)
)
for (i in seq_along(input_values)) {
  input <- input_values[[i]]
  target_name <- target_lst[i]
  est <- input$est
  se <- input$se
  lower <- est - 1.96 * se
  upper <- est + 1.96 * se
  vanilla.CI <- rbind(vanilla.CI, data.frame(target = target_name, est = est, se=se, lower = lower, upper = upper))
}
print(vanilla.CI)
```
...
ratio.CI<- data.frame(variable = character(0), est = numeric(0), se = numeric(0), lower = numeric(0), upper = numeric(0))
input_values <- list(
  ap_hi = list(est = ratio.ap_hi.est , se = ratio.ap_hi.se ),
  ap_lo = list(est = ratio.ap_lo.est, se = ratio.ap_lo.se),
  cardio = list(est = ratio.cardio.est, se = ratio.cardio.se)
)
for (i in seq_along(input_values)) {
  input <- input_values[[i]]
  target_name <- target_lst[i]
  ...
  lower <- est - 1.96 * se
  upper <- est + 1.96 * se
  ratio.CI <- rbind(ratio.CI, data.frame(target = target_name, est = est, se=se, lower = lower, upper = upper))
}
print(ratio.CI)
```
...
reg.ap_hi.lower<- 126.353 - 1.96 * 0.2296917
reg.ap_hi.upper <- 126.353 + 1.96 * 0.2296917
reg.ap_lo.lower <- 81.39437 - 1.96 * 0.1321236
reg.ap_lo.upper <- 81.39437 + 1.96 * 0.1321236
reg.cardio.lower <- 33543.01 - 1.96 * 496.8257
reg.cardio.upper <- 33543.01 + 1.96 * 496.8257
reg.CI <- data.frame(
  variable = c("reg.ap_hi", "reg.ap_lo", "reg.cardio"),
  est = c(126.353, 81.39437, 33543.01),
  se = c(0.2296917, 0.1321236, 496.8257),
  lower = c(reg.ap_hi.lower, reg.ap_lo.lower, reg.cardio.lower),
  upper = c(reg.ap_hi.upper, reg.ap_lo.upper, reg.cardio.upper)
)
print(reg.CI)
```
...
```{r Comparing estimators}
colnames(ratio.CI) <- colnames(vanilla.CI) <- colnames(reg.CI)
combined_table <- rbind(vanilla.CI, ratio.CI, reg.CI)
...
We now want to see how we much from each strata we should sample. In our case, cost of sampling in our case is constant, as our sampling process is simulated. We want to see if we can use proportional allocation, so we check to see if we can assume the variances of each strata are equal through the stratum-specific population variances:

```{r Check equal variance assumption}
