...
regression.est_and_se <- function(X, y, x.pop) {
  ...
  ret <- c()
  ret$est <- round(est, 3); ret$se <- round(se, 3)
  ret$lower <- round(est - 1.96*se, 3)
  ret$upper <- round(est + 1.96*se, 3)
  return(ret)
}
...
reg.cardio$lower <- N*reg.cardio$lower
reg.cardio$upper <- N*reg.cardio$upper
...
set.seed(124)
n.smoke <- round(prop.table(table(data$smoke)) * n, 0)
pop.smoke0 <- data[data$smoke==0,]; pop.smoke1 <- data[data$smoke==1,]
strata.sample <- rbind(pop.smoke0[sample(1:nrow(pop.smoke0), n.smoke[1]),],
                       pop.smoke1[sample(1:nrow(pop.smoke1), n.smoke[2]), ])
